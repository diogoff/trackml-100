...
Apparently, this solution performs particularly well at finding particles that do not come from the collision axis. Based on a [post-competition analysis](https://twitter.com/trackmllhc/status/1070339064094736390), David Rousseau (one of the organizers) [wrote](https://www.kaggle.com/c/trackml-particle-identification/discussion/69981#433908):
...
<p align="center"><img src="https://raw.githubusercontent.com/diogoff/trackml-100/master/images/frame_02.png" width="540"></p>
...
To reduce the computational burden and improve the results, we group particles that go through exactly the same sequence of detectors. These particles are said to have the same _route_.
...
To identify candidate tracks, we follow the same procedure as described above, but now using routes instead of the original particles, as illustrated in the following picture:
...
Basically, it sets up _n_ workers by distributing the event ids in a round-robin fashion across these workers.
...
Note, however, the memory requirement: 500 GB of RAM. (Again, it might be possible to alleviate this requirement - and significantly accelerate the computation time - by using something such as [Dask](https://dask.org/).)
...
The test hits that have been picked by a route are assigned the same track id. (A new track id is generated for each route.) Any test hits that are left unpicked by routes are assigned a track id of zero.
